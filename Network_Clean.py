...
class run:
    ...
    def plot_sphere_a(self, sph):
        ...
        self.surf = self.ax.plot_trisurf(self.x,self.y,self.z, triangles=sph.ch.simplices, cmap=plt.cm.Greys_r, alpha = 0.6)
...
def connectome(s):
    ...
    lc1 = Line3DCollection(linesv[:int(5*len(linesv)/7)], colors=colv, linewidths=3)
    lc2 = Line3DCollection(linesh[:int(5*len(linesh)/7)], colors=colh, linewidths=3)
...
s = sp.Sphere(vertices = icosahedron_vertices, faces = faces, recursion_level = 6 )
