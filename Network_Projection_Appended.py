...
class run:
    ...
    def plot_sphere_a(self, sph):
        ...
        #axes.set_xlim([-4,4])
        #axes.set_ylim([-5,35])
        self.xc, self.yc, self.z   = sph.ch.points[sph.ch.vertices][:,0],sph.ch.points[sph.ch.vertices][:,1], sph.ch.points[sph.ch.vertices][:,2]
        #print("self.x", self.x)
        #self.avg = (self.xc+ self.yc+ self.zc)/3
        self.x, self.y = sph.Mercator_Projection(np.pi/1.)#sph.Mercator_Projection()#s#Mollewide
